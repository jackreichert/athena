...
- **Full packages** (Standard or Deep) include both the `digests/` folder and `02-digests.md` (index + recommended reading order with links into the individual digests). **The Phase 5 integrity audit verifies that every digest file referenced by `02-digests.md` actually exists under `digests/` and that every Core source has a digest; missing files force a return to Phase 2.**
